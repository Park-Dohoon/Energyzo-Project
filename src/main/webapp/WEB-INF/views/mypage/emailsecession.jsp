...
function verifyAuthCode() { 
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            alert(response.message || "인증이 완료되었습니다."); // 서버에서 보낸 메시지 표시
        },
	...
	<main>
        <div class="container-fluid px-4">
          ...
          <div style="border: 1px solid #ddd; border-radius: 5px; padding: 20px; background-color: #f8f9fa; line-height: 1.6; color: #6c757d;">
            <!-- 이메일 인증 요청 -->
            <label for="birthdate">이메일 인증요청</label>
        ...
        <button type="button" class="auth-button" onclick="sendEmail()">인증요청</button>
              ...
              <!-- 인증번호 확인 -->
              <div class="form-group">
                <input type="text" id="auth-code" name="authCode" placeholder="인증번호 6자리" required>
                <button type="button" class="verify-button" onclick="verifyAuthCode()">인증번호확인</button>
              </div>
            </form>
          </div>
        </div>
